...
internal static class RawCdImageDetector
{
    ...
    internal static string? DetectTrackMode(string imagePath)
    {
        try
        {
            ...
            if (length == 0 || length % RawSectorSize != 0) return null;
            ...
            if (!header.AsSpan(0, SyncMark.Length).SequenceEqual(SyncMark)) return null;
    ...
    )
    {
        ...
        if (Path.IsPathRooted(relativeImagePath)) return null;
