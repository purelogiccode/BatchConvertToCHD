...
- **ISZ decompression** — UltraISO `.isz` images are decompressed in-process (zlib, bzip2, stored and all-zero chunks), including images split across `.i01`/`.i02` or `.part01.isz`/`.part001.isz` segments. The obfuscated tables, stripped bzip2 headers and no-chunk-table layout real files use are handled, UltraISO's checksum is validated when present, segments are matched by volume serial number, a missing one is named, and an encrypted image says so.
