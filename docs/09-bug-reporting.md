...
| **Encoder startup incompatibility** | `"terminated abnormally during the startup check"` — a negative exit code during the startup probe (0xC0000139 entry point not found, illegal instruction, …) means the bundled chdman build does not run on the user's CPU/Windows version |
